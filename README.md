...
- `jax` (recommended), `jax-cpu`, and `torch`: installs dependencies for the interactive Tactile MNIST environment with PyTorch, JAX, or JAX without CUDA support, respectively. Without any of those options, the interactive environment cannot be used but the static datasets will still work.
...
This package provides ap_gym environments for four active tactile perception environments: [TactileMNIST](docs/TactileMNIST.md), [Starstruck](docs/Starstruck.md), [Toolbox](docs/Toolbox.md), and [TactileMNISTVolume](docs/TactileMNISTVolume.md).
